...
model.mean <- function(D, params){
  mu =  exp(params[1])*D$y.neighbors + # neighbors
...
fit_freqGLMepi <- function(df, num_inits = 10, BFGS = T, verbose = T, target_col = 'y_imp', init = NULL){
  ...
  parnames = c('By.neighbors', 'Bintercept', 'Byear', 'Bcos1', 'Bsin1', 'Bcos2', 'Bsin2', 'Bcos3', 'Bsin3')
  names(init) = parnames
...
simulate_data_freqGLM_epi <- function(district_sizes, R = 1, lambda = -2, phi = -2, num_iters = 10, scale_by_num_neighbors = F, seed = 10, start_date = '2016-01-01', end_date = '2019-12-01', b0_mean = 7, b1_mean = -0.2, b1_sd = 0.2){
  ...
  for(r in 1:R){
    ...
    for(i in 1:num_iters){
      ...
      mean_y = exp(phi)*tmp$y.neighbors + exp(tmp$mu_seasonal)
  ...
  params_true = rbind(t(data.frame(By.neighbors = rep(phi, ncol(params_true)), row.names = colnames(params_true))), params_true)
