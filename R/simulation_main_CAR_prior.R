...
one_run <- function(lst, i, models = c('freq', 'WF', 'CAR')){
  ...
  if('CAR' %in% models){
    ...
    return_list <- tryCatch({
      res <- CARBayes_wrapper(return_list[['df_miss']], burnin = 10000, n.sample = 20000, prediction_sample = T, model = 'facility_fixed', predict_start_date = '2016-01-01', thin = 10, prior = 'none')
      colnames(res$df) <- gsub('y_pred_CAR_0', 'y_pred_CAR_none_0', colnames(res$df))
    ...
    return_list <- tryCatch({
      res <- CARBayes_wrapper(return_list[['df_miss']], burnin = 10000, n.sample = 20000, prediction_sample = T, model = 'facility_fixed', predict_start_date = '2016-01-01', thin = 10, prior = 'WF', prior_var_scale = 1)
      colnames(res$df) <- gsub('y_pred_CAR_0', 'y_pred_CAR_WF1_0', colnames(res$df))
      return_list[['df_miss']] <- res$df
      return_list[['district_df']] <- merge(return_list[['district_df']], res$district_df, by = c('district','date'))
      return_list[['CAR_summary2']] <- res$summary_stats
      return_list
    }, error = function(e){
      print(e)
      return_list[['errors']][['CARBayes']] <- rbind(return_list[['errors']][['CARBayes']], data.frame(i, error = e[[1]]))
    ...
    return_list <- tryCatch({
      res <- CARBayes_wrapper(return_list[['df_miss']], burnin = 10000, n.sample = 20000, prediction_sample = T, model = 'facility_fixed', predict_start_date = '2016-01-01', thin = 10, prior = 'WF', prior_var_scale = 10)
      colnames(res$df) <- gsub('y_pred_CAR_0', 'y_pred_CAR_WF10_0', colnames(res$df))
      return_list[['df_miss']] <- res$df
      return_list[['district_df']] <- merge(return_list[['district_df']], res$district_df, by = c('district','date'))
      return_list[['CAR_summary3']] <- res$summary_stats
      return_list
    }, error = function(e){
      print(e)
      return_list[['errors']][['CARBayes']] <- rbind(return_list[['errors']][['CARBayes']], data.frame(i, error = e[[1]]))
    ...
    return_list <- tryCatch({
      res <- CARBayes_wrapper(return_list[['df_miss']], burnin = 10000, n.sample = 20000, prediction_sample = T, model = 'facility_fixed', predict_start_date = '2016-01-01', thin = 10, prior = 'WF', prior_var_scale = 100)
      colnames(res$df) <- gsub('y_pred_CAR_0', 'y_pred_CAR_WF100_0', colnames(res$df))
      return_list[['df_miss']] <- res$df
      return_list[['district_df']] <- merge(return_list[['district_df']], res$district_df, by = c('district','date'))
      return_list[['CAR_summary4']] <- res$summary_stats
      return_list
    }, error = function(e){
      print(e)
      return_list[['errors']][['CARBayes']] <- rbind(return_list[['errors']][['CARBayes']], data.frame(i, error = e[[1]]))
    ...
    return_list <- tryCatch({
      res <- CARBayes_wrapper(return_list[['df_miss']], burnin = 10000, n.sample = 20000, prediction_sample = T, model = 'facility_fixed', predict_start_date = '2016-01-01', thin = 10, prior = 'constant', prior_mean = rep(0, 160), prior_var = rep(0.01, 160))
      colnames(res$df) <- gsub('y_pred_CAR_0', 'y_pred_CAR_constant_0', colnames(res$df))
      return_list[['df_miss']] <- res$df
      return_list[['district_df']] <- merge(return_list[['district_df']], res$district_df, by = c('district','date'))
      return_list[['CAR_summary5']] <- res$summary_stats
      return_list
    }, error = function(e){
      print(e)
      return_list[['errors']][['CARBayes']] <- rbind(return_list[['errors']][['CARBayes']], data.frame(i, error = e[[1]]))
