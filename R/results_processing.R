...
get_CARconvergence <- function(res, p = '0', CAR_name = 'CARstan_summary'){
  ...
  # only keep results that are there.
  indices <- sapply(CARsums, function(xx) ifelse(length(xx) > 0, 1, 0))
  
  CARsums <- CARsums[indices]
  ...
  theta_ind <- grep('theta',names(ESS))
  beta_ind <- setdiff(1:length(ESS), grep('tau2|rho|alpha|theta', names(ESS)))
  ESS <- c(ESS[1:3], 
           thetas = median(ESS[theta_ind]),
           betas = median(ESS[beta_ind]))
  ...
  # make rhat concise.
  rhat <- c(rhat[1:3], 
           thetas = median(rhat[theta_ind]),
           betas = median(rhat[beta_ind]))
...
get_correlation <- function(files, num_calc = 10, methods = c('y_pred_WF_negbin', 'y_pred_freqGLMepi_negbin','y_CAR_phifit'), normalize_weights = F){
  # get the results
  ...
  for(p in c('0','01','02','03','04', '05')){
    ...
    tmp_lst <- lapply(sample(length(tt), num_calc), function(ii){
      tmp <- tt[[ii]]
      return(list(moran = get_morans_I(tmp, normalize_weights = normalize_weights), alpha = get_temporal_autocorrelation(tmp)))
    })
...
#### Plotting all appendix results (07/29/2024) ####
load('C:/Users/Admin-Dell/Dropbox/Academic/HSPH/Research/Syndromic Surveillance/results/appendix_results_allNB_07292024.RData')

DGP_ylims = list(ylim(0,1), ylim(0,1), ylim(0, 1), ylim(0,1))
DGP_ylims_half = list(ylim(0.5,1), ylim(0.5,1), ylim(0.5, 1), ylim(0.5,1))
DGP_ylims_3 = list(ylim(0.5,1), ylim(0,1), ylim(0.5, 1), ylim(0.5,1))

# Facility figure beta0 = 6, beta1 = 0
{
  p1 <-   plot_all_methods(res = res_facility[["WF_beta06_beta10"]]$results, methods = c('WF', 'freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims_half, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with WF model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p2 <- plot_all_methods(res = res_facility[["freqGLM_beta055_beta10"]]$results, methods = c('WF', 'freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims_half, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with freqGLM model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p3 <- plot_all_methods(res = res_facility[["CAR_beta06_beta10"]]$results, methods = c('WF', 'freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims_half, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with CAR model', include_legend = F, squeeze_plots = T, remove_x_axis = F, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  cowplot::plot_grid(p1$plot, p2$plot, p3$plot, p1$legend, ncol = 1, rel_heights = c(3,3,3.55,1))
  #cowplot::plot_grid(p1$plot, p2$plot,  p1$legend, ncol = 1, rel_heights = c(3,3,1))
  ggsave('figures/MCAR_beta06_beta10_facility_plots_07292024.pdf', height = 7.5, width = 10)
}

# District figure beta0 = 6, beta1 = 0
{
  p1 <-   plot_all_methods(res = res_district[["WF_beta06_beta10"]]$results, methods = c('WF', 'freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with WF model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p2 <- plot_all_methods(res = res_district[["freqGLM_beta055_beta10"]]$results, methods = c('WF', 'freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with freqGLM model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p3 <- plot_all_methods(res = res_district[["CAR_beta06_beta10"]]$results, methods = c('WF', 'freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with CAR model', include_legend = F, squeeze_plots = T, remove_x_axis = F, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  cowplot::plot_grid(p1$plot, p2$plot, p3$plot, p1$legend, ncol = 1, rel_heights = c(3,3,3.55,1))
  #cowplot::plot_grid(p1$plot, p2$plot,  p1$legend, ncol = 1, rel_heights = c(3,3,1))
  ggsave('figures/MCAR_beta06_beta10_district_plots_07292024.pdf', height = 7.5, width = 10)
}

# Facility figure beta0 = 2, beta1 = 0
{
  p1 <-   plot_all_methods(res = res_facility[["WF_beta02_beta10"]]$results, methods = c('WF', 'freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims_3, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with WF model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p2 <- plot_all_methods(res = res_facility[["freqGLM_beta015_beta10"]]$results, methods = c('WF', 'freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims_3, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with freqGLM model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p3 <- plot_all_methods(res = res_facility[["CAR_beta02_beta10"]]$results, methods = c('WF', 'freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims_3, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with CAR model', include_legend = F, squeeze_plots = T, remove_x_axis = F, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  cowplot::plot_grid(p1$plot, p2$plot, p3$plot, p1$legend, ncol = 1, rel_heights = c(3,3,3.55,1))
  #cowplot::plot_grid(p1$plot, p2$plot,  p1$legend, ncol = 1, rel_heights = c(3,3,1))
  ggsave('figures/MCAR_beta02_beta10_facility_plots_07292024.pdf', height = 7.5, width = 10)
}

# District figure beta0 = 2, beta1 = 0
{
  p1 <-   plot_all_methods(res = res_district[["WF_beta02_beta10"]]$results, methods = c('WF', 'freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with WF model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p2 <- plot_all_methods(res = res_district[["freqGLM_beta015_beta10"]]$results, methods = c('WF', 'freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with freqGLM model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p3 <- plot_all_methods(res = res_district[["CAR_beta02_beta10"]]$results, methods = c('WF', 'freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with CAR model', include_legend = F, squeeze_plots = T, remove_x_axis = F, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  cowplot::plot_grid(p1$plot, p2$plot, p3$plot, p1$legend, ncol = 1, rel_heights = c(3,3,3.55,1))
  #cowplot::plot_grid(p1$plot, p2$plot,  p1$legend, ncol = 1, rel_heights = c(3,3,1))
  ggsave('figures/MCAR_beta02_beta10_district_plots_07292024.pdf', height = 7.5, width = 10)
}

# Facility figure more dispersion
{
  p1 <-   plot_all_methods(res = res_facility[["WF_DGPthetarate0667"]]$results, methods = c('WF', 'freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims_3, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with WF model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p2 <- plot_all_methods(res = res_facility[["freqGLM_DGPthetarate0667"]]$results, methods = c('WF', 'freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims_3, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with freqGLM model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p3 <- plot_all_methods(res = res_facility[["CAR_DGPthetarate0667"]]$results, methods = c('WF', 'freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims_3, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with CAR model', include_legend = F, squeeze_plots = T, remove_x_axis = F, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  cowplot::plot_grid(p1$plot, p2$plot, p3$plot, p1$legend, ncol = 1, rel_heights = c(3,3,3.55,1))
  #cowplot::plot_grid(p1$plot, p2$plot,  p1$legend, ncol = 1, rel_heights = c(3,3,1))
  ggsave('figures/MCAR_DGPthetarate0667_facility_plots_07292024.pdf', height = 7.5, width = 10)
}

# District figure more dispersion
{
  p1 <-   plot_all_methods(res = res_district[["WF_DGPthetarate0667"]]$results, methods = c('WF', 'freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with WF model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p2 <- plot_all_methods(res = res_district[["freqGLM_DGPthetarate0667"]]$results, methods = c('WF', 'freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with freqGLM model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p3 <- plot_all_methods(res = res_district[["CAR_DGPthetarate0667"]]$results, methods = c('WF', 'freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with CAR model', include_legend = F, squeeze_plots = T, remove_x_axis = F, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  cowplot::plot_grid(p1$plot, p2$plot, p3$plot, p1$legend, ncol = 1, rel_heights = c(3,3,3.55,1))
  #cowplot::plot_grid(p1$plot, p2$plot,  p1$legend, ncol = 1, rel_heights = c(3,3,1))
  ggsave('figures/MCAR_DGPthetarate0667_district_plots_07292024.pdf', height = 7.5, width = 10)
}

# Facility figure more spatial correlation
{
  p1 <-   plot_all_methods(res = res_facility[["freqGLM_0402"]]$results, methods = c('WF', 'freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims_3, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with freqGLM model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p2 <- plot_all_methods(res = res_facility[["CAR_07031"]]$results, methods = c('WF', 'freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims_3, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with CAR model', include_legend = F, squeeze_plots = T, remove_x_axis = F, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  cowplot::plot_grid(p1$plot, p2$plot, p1$legend, ncol = 1, rel_heights = c(3,3.55,0.5))
  #cowplot::plot_grid(p1$plot, p2$plot,  p1$legend, ncol = 1, rel_heights = c(3,3,1))
  ggsave('figures/MCAR_higher_spatial_facility_plots_07292024.pdf', height = 5, width = 10)
}

# District figure more spatial correlation
{
  p1 <-   plot_all_methods(res = res_district[["freqGLM_0402"]]$results, methods = c('WF', 'freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with freqGLM model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p2 <- plot_all_methods(res = res_district[["CAR_07031"]]$results, methods = c('WF', 'freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with CAR model', include_legend = F, squeeze_plots = T, remove_x_axis = F, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  cowplot::plot_grid(p1$plot, p2$plot, p1$legend, ncol = 1, rel_heights = c(3,3.55,0.5))
  #cowplot::plot_grid(p1$plot, p2$plot,  p1$legend, ncol = 1, rel_heights = c(3,3,1))
  ggsave('figures/MCAR_higher_spatial_district_plots_07292024.pdf', height = 5, width = 10)
}

#
#### Processing all appendix results (07/29/2024) ####
files_07_16 <- grep('2024_07_16', dir(res_dir, full.names = T), value = T)
# length 108, which is good.
# groups of files by DGP.
WF_files <- grep('wf', files_07_16, value = T)
freqGLM_files <- grep('freqglm', files_07_16, value = T)
CAR_files <- grep('_car', files_07_16, value = T)

# there should be 11 simulations (66 files) for the appendix
# (1) WF beta0 = 6, beta1 = 0
WF_beta06_beta10 <- grep('beta06_beta10', WF_files, value = T)
  
# (2) freqGLM beta0 = 5.5, beta1 = 0
freqGLM_beta055_beta10 <- grep('beta055_beta10', freqGLM_files, value = T)

# (3) CAR beta0 = 6, beta1 = 0
CAR_beta06_beta10 <- grep('beta06_beta10', CAR_files, value = T)

# (4) WF beta0 = 2, beta1 = 0
WF_beta02_beta10 <- grep('beta02_beta10', WF_files, value = T)

# (5) freqGLM beta0 = 1.5, beta1 = 0
freqGLM_beta015_beta10 <- grep('beta015_beta10', freqGLM_files, value = T)

# (6) CAR beta0 = 2, beta1 = 0
CAR_beta02_beta10 <- grep('beta02_beta10', CAR_files, value = T)

# (7) CAR with higher spatial correlation (rho = 0.7 and tau2 = 1)
CAR_07031 <- grep('07031', CAR_files, value = T)

# (8) freqGLM with higher spatial correlation (rho = 0.4, alpha = 0.2)
freqGLM_0402 <- grep('0402', freqGLM_files, value = T)

# (9) WF with more overdispersion (higher theta rate)
WF_DGPthetarate0667 <- grep('dgpthetarate0667', WF_files, value = T)

# (10) freqGLM with more overdispersion (higher theta rate)
freqGLM_DGPthetarate0667 <- grep('dgpthetarate0667', freqGLM_files, value = T)

# (11) CAR with more overdispersion (higher theta rate)
CAR_DGPthetarate0667 <- grep('dgpthetarate0667', CAR_files, value = T)

file_name_str <- c('WF_beta06_beta10', 'freqGLM_beta055_beta10', 'CAR_beta06_beta10',
                   'WF_beta02_beta10', 'freqGLM_beta015_beta10', 'CAR_beta02_beta10',
                   'CAR_07031', 'freqGLM_0402', 
                   'WF_DGPthetarate0667', 'freqGLM_DGPthetarate0667', 'CAR_DGPthetarate0667')

# to test the match
for(nn in file_name_str){
  get(nn)
}
...
methods <- c('y_pred_WF_negbin', 'y_pred_freqGLMepi_negbin','y_CAR_phifit_negbin')
rename_vec <- c('WF', 'freqGLM', 'CAR')

# grab the results from all runs
for(name in file_name_str){
  res_facility[[name]] <- get_results(get(name), name, expected_sims = NULL, methods = methods, rename_vec = rename_vec, give_method_name_err = T, ignore_size_err = T)
  res_district[[name]] <-  get_results(get(name), name,  district_results = T, expected_sims = NULL, methods = methods, rename_vec = rename_vec, give_method_name_err = T, ignore_size_err = T)
}
# mostly all good. The CAR_07031 has some issues where a good chunk of simulations are not completeing (up to 18%). Why? Not going to look into that right now.

# save(res_facility, res_district, file = 'C:/Users/Admin-Dell/Dropbox/Academic/HSPH/Research/Syndromic Surveillance/results/appendix_results_allNB_07292024.RData')

#
#
#### Plotting the 9 NB main paper results - grouped by DGP (07/23/2024) ####
load('C:/Users/Admin-Dell/Dropbox/Academic/HSPH/Research/Syndromic Surveillance/results/main_paper_results_allNB_07222024.RData')
...
# WF facility plots
{
  p1 <-   plot_all_methods(res = res_facility[["WF_MCAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'), method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims_half, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity:k=3', 'sensitivity:k=5', 'sensitivity:k=10'), results_by_point = F, rows = 1, title = 'MCAR', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p2 <- plot_all_methods(res = res_facility[["WF_MAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims_half, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity:k=3', 'sensitivity:k=5', 'sensitivity:k=10'), results_by_point = F, rows = 1, title = 'MAR', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p3 <- plot_all_methods(res = res_facility[["WF_MNAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'), add_lines = list(F, F, F, F),  fix_axis = DGP_ylims_half, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity:k=3', 'sensitivity:k=5', 'sensitivity:k=10'), results_by_point = F, rows = 1, title = 'MNAR', include_legend = F, squeeze_plots = T, remove_x_axis = F, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  cowplot::plot_grid(p1$plot, p2$plot, p3$plot, p1$legend, ncol = 1, rel_heights = c(3,3,3.55,1))
  #cowplot::plot_grid(p1$plot, p2$plot,  p1$legend, ncol = 1, rel_heights = c(3,3,1))
  ggsave('figures/WF_facility_plots_07222024.pdf', height = 7.5, width = 10)
}

# WF district plots
{
  p1 <-   plot_all_methods(res = res_district[["WF_MCAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'), method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity:k=3', 'sensitivity:k=5', 'sensitivity:k=10'), results_by_point = F, rows = 1, title = 'MCAR', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p2 <- plot_all_methods(res = res_district[["WF_MAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity:k=3', 'sensitivity:k=5', 'sensitivity:k=10'), results_by_point = F, rows = 1, title = 'MAR', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p3 <- plot_all_methods(res = res_district[["WF_MNAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'), add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity:k=3', 'sensitivity:k=5', 'sensitivity:k=10'), results_by_point = F, rows = 1, title = 'MNAR', include_legend = F, squeeze_plots = T, remove_x_axis = F, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  cowplot::plot_grid(p1$plot, p2$plot, p3$plot, p1$legend, ncol = 1, rel_heights = c(3,3,3.55,1))
  #cowplot::plot_grid(p1$plot, p2$plot,  p1$legend, ncol = 1, rel_heights = c(3,3,1))
  ggsave('figures/WF_district_plots_07222024.pdf', height = 7.5, width = 10)
}

# freqGLM facility plots
{
  p1 <-   plot_all_methods(res = res_facility[["freqGLM_MCAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'), method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims_half, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity:k=3', 'sensitivity:k=5', 'sensitivity:k=10'), results_by_point = F, rows = 1, title = 'MCAR', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p2 <- plot_all_methods(res = res_facility[["freqGLM_MAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims_half, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity:k=3', 'sensitivity:k=5', 'sensitivity:k=10'), results_by_point = F, rows = 1, title = 'MAR', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p3 <- plot_all_methods(res = res_facility[["freqGLM_MNAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'), add_lines = list(F, F, F, F),  fix_axis = DGP_ylims_half, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity:k=3', 'sensitivity:k=5', 'sensitivity:k=10'), results_by_point = F, rows = 1, title = 'MNAR', include_legend = F, squeeze_plots = T, remove_x_axis = F, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  cowplot::plot_grid(p1$plot, p2$plot, p3$plot, p1$legend, ncol = 1, rel_heights = c(3,3,3.55,1))
  #cowplot::plot_grid(p1$plot, p2$plot,  p1$legend, ncol = 1, rel_heights = c(3,3,1))
  ggsave('figures/freqGLM_facility_plots_07222024.pdf', height = 7.5, width = 10)
}

# freqGLM district plots
{
  p1 <-   plot_all_methods(res = res_district[["freqGLM_MCAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'), method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity:k=3', 'sensitivity:k=5', 'sensitivity:k=10'), results_by_point = F, rows = 1, title = 'MCAR', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p2 <- plot_all_methods(res = res_district[["freqGLM_MAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity:k=3', 'sensitivity:k=5', 'sensitivity:k=10'), results_by_point = F, rows = 1, title = 'MAR', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p3 <- plot_all_methods(res = res_district[["freqGLM_MNAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'), add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity:k=3', 'sensitivity:k=5', 'sensitivity:k=10'), results_by_point = F, rows = 1, title = 'MNAR', include_legend = F, squeeze_plots = T, remove_x_axis = F, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  cowplot::plot_grid(p1$plot, p2$plot, p3$plot, p1$legend, ncol = 1, rel_heights = c(3,3,3.55,1))
  #cowplot::plot_grid(p1$plot, p2$plot,  p1$legend, ncol = 1, rel_heights = c(3,3,1))
  ggsave('figures/freqGLM_district_plots_07222024.pdf', height = 7.5, width = 10)
}

# CAR facility plots
{
  p1 <-   plot_all_methods(res = res_facility[["CAR_MCAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'), method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims_half, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity:k=3', 'sensitivity:k=5', 'sensitivity:k=10'), results_by_point = F, rows = 1, title = 'MCAR', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p2 <- plot_all_methods(res = res_facility[["CAR_MAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims_half, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity:k=3', 'sensitivity:k=5', 'sensitivity:k=10'), results_by_point = F, rows = 1, title = 'MAR', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p3 <- plot_all_methods(res = res_facility[["CAR_MNAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'), add_lines = list(F, F, F, F),  fix_axis = DGP_ylims_half, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity:k=3', 'sensitivity:k=5', 'sensitivity:k=10'), results_by_point = F, rows = 1, title = 'MNAR', include_legend = F, squeeze_plots = T, remove_x_axis = F, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  cowplot::plot_grid(p1$plot, p2$plot, p3$plot, p1$legend, ncol = 1, rel_heights = c(3,3,3.55,1))
  #cowplot::plot_grid(p1$plot, p2$plot,  p1$legend, ncol = 1, rel_heights = c(3,3,1))
  ggsave('figures/CAR_facility_plots_07222024.pdf', height = 7.5, width = 10)
}

# CAR district plots
{
  p1 <-   plot_all_methods(res = res_district[["CAR_MCAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'), method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity:k=3', 'sensitivity:k=5', 'sensitivity:k=10'), results_by_point = F, rows = 1, title = 'MCAR', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p2 <- plot_all_methods(res = res_district[["CAR_MAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity:k=3', 'sensitivity:k=5', 'sensitivity:k=10'), results_by_point = F, rows = 1, title = 'MAR', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p3 <- plot_all_methods(res = res_district[["CAR_MNAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'), add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity:k=3', 'sensitivity:k=5', 'sensitivity:k=10'), results_by_point = F, rows = 1, title = 'MNAR', include_legend = F, squeeze_plots = T, remove_x_axis = F, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  cowplot::plot_grid(p1$plot, p2$plot, p3$plot, p1$legend, ncol = 1, rel_heights = c(3,3,3.55,1))
  #cowplot::plot_grid(p1$plot, p2$plot,  p1$legend, ncol = 1, rel_heights = c(3,3,1))
  ggsave('figures/CAR_district_plots_07222024.pdf', height = 7.5, width = 10)
}

#
#### Calculate the aggregated main paper results numbers (07/22/2024) ####
load('C:/Users/Admin-Dell/Dropbox/Academic/HSPH/Research/Syndromic Surveillance/results/main_paper_results_allNB_07222024.RData')
...
#
#### Plotting the 9 NB main paper results (07/22/2024) ####
load('C:/Users/Admin-Dell/Dropbox/Academic/HSPH/Research/Syndromic Surveillance/results/main_paper_results_allNB_07222024.RData')
...
# MCAR facility plots
{
  p1 <-   plot_all_methods(res = res_facility[["WF_MCAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'), method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with WF model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p2 <- plot_all_methods(res = res_facility[["freqGLM_MCAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with freqGLM model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p3 <- plot_all_methods(res = res_facility[["CAR_MCAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'), add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with CAR model', include_legend = F, squeeze_plots = T, remove_x_axis = F, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  cowplot::plot_grid(p1$plot, p2$plot, p3$plot, p1$legend, ncol = 1, rel_heights = c(3,3,3.55,1))
  #cowplot::plot_grid(p1$plot, p2$plot,  p1$legend, ncol = 1, rel_heights = c(3,3,1))
  ggsave('figures/MCAR_facility_plots_07222024.pdf', height = 7.5, width = 10)
}

# MCAR district plots
{
  p1 <-   plot_all_methods(res = res_district[["WF_MCAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'), method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with WF model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p2 <- plot_all_methods(res = res_district[["freqGLM_MCAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with freqGLM model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  p3 <- plot_all_methods(res = res_district[["CAR_MCAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'), add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with CAR model', include_legend = F, squeeze_plots = T, remove_x_axis = F, keep_y_axis = 'all')#, y_lab = 'MCAR')
  
  cowplot::plot_grid(p1$plot, p2$plot, p3$plot, p1$legend, ncol = 1, rel_heights = c(3,3,3.55,1))
  #cowplot::plot_grid(p1$plot, p2$plot,  p1$legend, ncol = 1, rel_heights = c(3,3,1))
  ggsave('figures/MCAR_district_plots_07222024.pdf', height = 7.5, width = 10)
}

# MAR facility plots
{
  p1 <-   plot_all_methods(res = res_facility[["WF_MAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'), method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with WF model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MAR')
  
  p2 <- plot_all_methods(res = res_facility[["freqGLM_MAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with freqGLM model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MAR')
  
  p3 <- plot_all_methods(res = res_facility[["CAR_MAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'), add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with CAR model', include_legend = F, squeeze_plots = T, remove_x_axis = F, keep_y_axis = 'all')#, y_lab = 'MAR')
  
  cowplot::plot_grid(p1$plot, p2$plot, p3$plot, p1$legend, ncol = 1, rel_heights = c(3,3,3.55,1))
  #cowplot::plot_grid(p1$plot, p2$plot,  p1$legend, ncol = 1, rel_heights = c(3,3,1))
  ggsave('figures/MAR_facility_plots_07222024.pdf', height = 7.5, width = 10)
}

# MAR district plots
{
  p1 <-   plot_all_methods(res = res_district[["WF_MAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'), method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with WF model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MAR')
  
  p2 <- plot_all_methods(res = res_district[["freqGLM_MAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with freqGLM model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MAR')
  
  p3 <- plot_all_methods(res = res_district[["CAR_MAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'), add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with CAR model', include_legend = F, squeeze_plots = T, remove_x_axis = F, keep_y_axis = 'all')#, y_lab = 'MAR')
  
  cowplot::plot_grid(p1$plot, p2$plot, p3$plot, p1$legend, ncol = 1, rel_heights = c(3,3,3.55,1))
  #cowplot::plot_grid(p1$plot, p2$plot,  p1$legend, ncol = 1, rel_heights = c(3,3,1))
  ggsave('figures/MAR_district_plots_07222024.pdf', height = 7.5, width = 10)
}

# MNAR facility plots
{
  p1 <-   plot_all_methods(res = res_facility[["WF_MNAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'), method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with WF model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MNAR')
  
  p2 <- plot_all_methods(res = res_facility[["freqGLM_MNAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with freqGLM model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MNAR')
  
  p3 <- plot_all_methods(res = res_facility[["CAR_MNAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'), add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with CAR model', include_legend = F, squeeze_plots = T, remove_x_axis = F, keep_y_axis = 'all')#, y_lab = 'MNAR')
  
  cowplot::plot_grid(p1$plot, p2$plot, p3$plot, p1$legend, ncol = 1, rel_heights = c(3,3,3.55,1))
  #cowplot::plot_grid(p1$plot, p2$plot,  p1$legend, ncol = 1, rel_heights = c(3,3,1))
  ggsave('figures/MNAR_facility_plots_07222024.pdf', height = 7.5, width = 10)
}

# MNAR district plots
{
  p1 <-   plot_all_methods(res = res_district[["WF_MNAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'), method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with WF model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MNAR')
  
  p2 <- plot_all_methods(res = res_district[["freqGLM_MNAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'),  add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with freqGLM model', include_legend = F, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')#, y_lab = 'MNAR')
  
  p3 <- plot_all_methods(res = res_district[["CAR_MNAR"]]$results, methods = c('WF_NB', 'freqGLM_NB','CAR_NB'),  method_rename = c('WF','freqGLM','CAR'), add_lines = list(F, F, F, F),  fix_axis = DGP_ylims, metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity-3', 'sensitivity-5', 'sensitivity-10'), results_by_point = F, rows = 1, title = 'Data Generated with CAR model', include_legend = F, squeeze_plots = T, remove_x_axis = F, keep_y_axis = 'all')#, y_lab = 'MNAR')
  
  cowplot::plot_grid(p1$plot, p2$plot, p3$plot, p1$legend, ncol = 1, rel_heights = c(3,3,3.55,1))
  #cowplot::plot_grid(p1$plot, p2$plot,  p1$legend, ncol = 1, rel_heights = c(3,3,1))
  ggsave('figures/MNAR_district_plots_07222024.pdf', height = 7.5, width = 10)
}

#### Processing all 9 main paper results DISTRICT by point (07/22/2024) ####
# get the files
{
  files_OG_MCAR <- grep('2024_07_10', dir(res_dir, full.names = T), value = T)
  files_07_16 <- grep('2024_07_16', dir(res_dir, full.names = T), value = T)
  
  WF_MCAR <- grep('wf', files_OG_MCAR, value = T)
  freqGLM_MCAR <- grep('mcar[0-9]{1,2}_freqglm0202_beta055_beta1n025', files_07_16, value = T)
  CAR_MCAR <- grep('2024_07_11', dir(res_dir, full.names = T), value = T)
  
  WF_MAR <- grep('mar[0-9]{1,2}_wf_beta06_beta1n025', files_07_16, value = T)
  freqGLM_MAR <- grep('mar[0-9]{1,2}_freqglm0202_beta055_beta1n025', files_07_16, value = T)
  CAR_MAR <- grep('mar[0-9]{1,2}_car0303025_beta06_beta1n025', files_07_16, value = T)
  
  WF_MNAR <- grep('mnar[0-9]{1,2}_wf_beta06_beta1n025', files_07_16, value = T)
  freqGLM_MNAR <- grep('mnar[0-9]{1,2}_freqglm0202_beta055_beta1n025', files_07_16, value = T)
  CAR_MNAR <- grep('mnar[0-9]{1,2}_car0303025_beta06_beta1n025', files_07_16, value = T)
  
  file_name_str <- c('WF_MCAR', 'freqGLM_MCAR', 'CAR_MCAR',
                     'WF_MAR', 'freqGLM_MAR', 'CAR_MAR',
                     'WF_MNAR', 'freqGLM_MNAR', 'CAR_MNAR')
}

# set names of the models
rename_vec <- c('WF', 'freqGLM', 'CAR')
...
# grab the results from all runs
for(name in file_name_str){
  if(name %in% c('WF_MCAR', 'CAR_MCAR')){
    methods <- c('y_pred_WF_negbin', 'y_pred_freqGLMepi_negbin','y_CAR_phifit') 
  }else{
    methods <- c('y_pred_WF_negbin', 'y_pred_freqGLMepi_negbin','y_CAR_phifit_negbin')
  }
  res_district[[name]] <-  get_results(get(name), name,  district_results = T, results_by_point = T, expected_sims = NULL, methods = methods, rename_vec = rename_vec)
}

#save(res_district, file = 'C:/Users/Admin-Dell/Dropbox/Academic/HSPH/Research/Syndromic Surveillance/results/main_paper_district_by_point_results_07222024.RData')

text_size = 12
ylims = list(ylim(0.5,1), ylim(0.5,1), ylim(0.5, 1), ylim(0.5,1))
## DGP district plots

{
  p1 <- plot_all_methods(res = res_district[["WF_MCAR"]]$results, fix_axis = ylims, add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity:k=3', 'sensitivity:k=5', 'sensitivity:k=10'), results_by_point = F, rows = 1, title = 'Data Generated with WF model, assuming MCAR', include_legend = F, plot_indiv_points = T, legend_text = text_size, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')
  
  p2 <- plot_all_methods(res = res_district[["freqGLM_MCAR"]]$results, fix_axis = ylims, add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity:k=3', 'sensitivity:k=5', 'sensitivity:k=10'), results_by_point = F, rows = 1, title = 'Data Generated with freqGLM model, assuming MCAR', include_legend = F, plot_indiv_points = T, legend_text = text_size, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')
  
  p3 <- plot_all_methods(res = res_district[["CAR_MCAR"]]$results, fix_axis = ylims, add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3', 'outbreak_detection5', 'outbreak_detection10'), metric_rename = c('specificity', 'sensitivity:k=3', 'sensitivity:k=5', 'sensitivity:k=10'), results_by_point = F, rows = 1, title = 'Data Generated with CAR model, assuming MCAR', include_legend = F, plot_indiv_points = T, legend_text = text_size, squeeze_plots = T, remove_x_axis = T, keep_y_axis = 'all')
  
  cowplot::plot_grid(p1$plot, p2$plot, p3$plot, p1$legend, ncol = 1, rel_heights = c(3,3,3,1))
  # ggsave('figures/DGP_district_bypoint_comparison_plots_07222024.pdf', height = 7.5, width = 10)
}

#
#### Processing all 9 main paper results - NB DGP and fit (07/22/2024) ####
files_OG_MCAR <- grep('2024_07_10', dir(res_dir, full.names = T), value = T)
files_07_16 <- grep('2024_07_16', dir(res_dir, full.names = T), value = T)
...
# Just testing - did WF and CAR MCAR do negbin or not?
if(F){
  load('C:/Users/Admin-Dell/Dropbox/Academic/HSPH/Research/Syndromic Surveillance/results/mcar0_wf_beta06_beta1n025_id596482_2024_07_10/sim_results_p0.0_mcar_1(50).RData')
  res <- imputed_list[[1]]$CAR_phifit_negbin_summary
  # Yes! Phew
  load('C:/Users/Admin-Dell/Dropbox/Academic/HSPH/Research/Syndromic Surveillance/results/mcar0_car0303025_beta_06_beta1n025_negbin_2024_07_11/sim_results_p0.0_mcar_1(50).RData')
  res <- imputed_list[[1]]$CAR_phifit_negbin_summary
  # Yes! Also phew.
}
...
# methods <- c('y_pred_WF_negbin', 'y_pred_freqGLMepi_negbin','y_CAR_phifit_negbin')
rename_vec <- c('WF_NB', 'freqGLM_NB', 'CAR_NB')

# grab the results from all runs
for(name in file_name_str){
  # for the first two names, I hadn't renamed the CAR_phifit but I still did run CAR NB.
  if(name %in% c('WF_MCAR', 'CAR_MCAR')){
    methods <- c('y_pred_WF_negbin', 'y_pred_freqGLMepi_negbin','y_CAR_phifit') 
  }else{
    methods <- c('y_pred_WF_negbin', 'y_pred_freqGLMepi_negbin','y_CAR_phifit_negbin')
...
# save(res_facility, res_district, file = 'C:/Users/Admin-Dell/Dropbox/Academic/HSPH/Research/Syndromic Surveillance/results/main_paper_results_allNB_07222024.RData')

#
#### Processing CAR convergence of all the runs (updated 07/22/2024 ####
files_OG_MCAR <- grep('2024_07_10', dir(res_dir, full.names = T), value = T)
files_07_16 <- grep('2024_07_16', dir(res_dir, full.names = T), value = T)

WF_MCAR <- grep('wf', files_OG_MCAR, value = T)
freqGLM_MCAR <- grep('mcar[0-9]{1,2}_freqglm0202_beta055_beta1n025', files_07_16, value = T)
freqGLM_MCAR <- freqGLM_MCAR[-grep('dgptheta', freqGLM_MCAR)]
CAR_MCAR <- grep('2024_07_11', dir(res_dir, full.names = T), value = T)
...
# grab the results from all runs
for(name in file_name_str){
  res_f <- get_results(get(name), name, return_unprocessed = T, methods = methods, expected_sims = NULL)
  for(p in c('0','01','02','03','04','05')){
    res_facility[[name]][[p]] <- get_CARconvergence(res = res_f, p = p, CAR_name = 'CAR_phifit_negbin_summary')
  }
  
  res_d <- get_results(get(name), name,  district_results = T, return_unprocessed = T, methods = methods, expected_sims = NULL) 
   
  for(p in c('0','01','02','03','04','05')){
    res_district[[name]][[p]] <- get_CARconvergence(res = res_d, p = p, CAR_name = 'CAR_phifit_negbin_summary')
  }
}

# save(res_facility, res_district, file = 'C:/Users/Admin-Dell/Dropbox/Academic/HSPH/Research/Syndromic Surveillance/results/CAR_convergence_07292024.RData')

# get the results.
df_rhat <- NULL
for(n in names(res_facility)){
  tmp <- res_facility[[n]]
  for(p in names(tmp)){
    vec <- paste0(signif(tmp[[p]]$rhat, 3), ' (', signif(tmp[[p]]$ESS, 2), ')')
    df_tmp <- data.frame(DGP = n,
                         proportion_missing = as.numeric(p)/10,
                         tau2 = vec[1],
                         rho = vec[2],
                         alpha = vec[3],
                         thetas = vec[4],
                         betas = vec[5])
    df_rhat <- rbind(df_rhat, df_tmp)
  }
}

df_rhat$DGP <- gsub('_MCAR','',df_rhat$DGP)
#colnames(df_rhat) <- gsub('proportion_missing', 'p', colnames(df_rhat))

# save into an excel file!
write.csv(df_rhat, file = 'figures/MCAR_convergence_stats.csv', row.names = F, )
...
system.time({get_correlation(WF, num_calc = 10)})
...
file_name_str <- c('WF','freqGLM', 'CAR')
...
# facility plots
{
  p1 <-   plot_all_methods(res = res_facility[["WF"]]$results, fix_axis = DGP_ylims, methods = c('WF_NB', 'freqGLM_NB','CAR'),  add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, title = 'Data Generated with WF NB model', include_legend = F, squeeze_plots = T, remove_x_axis = T, remove_y_axis = F, y_lab = 'MCAR')
  
  p2 <- plot_all_methods(res = res_facility[["freqGLM"]]$results, fix_axis = DGP_ylims, methods = c('WF_NB', 'freqGLM_NB','CAR'),  add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, title = 'Data Generated with freqGLM NB model', include_legend = F, squeeze_plots = T, remove_x_axis = T, remove_y_axis = F, y_lab = 'MCAR')
  
  p3 <- plot_all_methods(res = res_facility[["CAR"]]$results, fix_axis = DGP_ylims, methods = c('WF_NB', 'freqGLM_NB','CAR'),  add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, title = 'Data Generated with CAR model', include_legend = F, squeeze_plots = T, remove_x_axis = F, remove_y_axis = F, y_lab = 'MCAR')
...
# facility plots
{
  p1 <-   plot_all_methods(res = res_facility[["WF_NB"]]$results, fix_axis = DGP_ylims, methods = c('WF_NB', 'freqGLM_NB','CAR'),  add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, title = 'Data Generated with WF NB model', include_legend = F, squeeze_plots = T, remove_x_axis = T, remove_y_axis = F, y_lab = 'MCAR')
  
  p2 <- plot_all_methods(res = res_facility[["freqGLM_NB"]]$results, fix_axis = DGP_ylims, methods = c('WF_NB', 'freqGLM_NB','CAR'),  add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, title = 'Data Generated with freqGLM NB model', include_legend = F, squeeze_plots = T, remove_x_axis = T, remove_y_axis = F, y_lab = 'MCAR')
...
# district plots
{
  p1 <-   plot_all_methods(res = res_district[["WF_NB"]]$results, fix_axis = DGP_ylims, methods = c('WF_NB', 'freqGLM_NB','CAR'),  add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, title = 'Data Generated with WF NB model', include_legend = F, squeeze_plots = T, remove_x_axis = T, remove_y_axis = F, y_lab = 'MCAR')
  
  p2 <- plot_all_methods(res = res_district[["freqGLM_NB"]]$results, fix_axis = DGP_ylims, methods = c('WF_NB', 'freqGLM_NB','CAR'),  add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, title = 'Data Generated with freqGLM NB model', include_legend = F, squeeze_plots = T, remove_x_axis = T, remove_y_axis = F, y_lab = 'MCAR')
  
  p3 <- plot_all_methods(res = res_district[["CAR"]]$results, fix_axis = DGP_ylims, methods = c('WF_NB', 'freqGLM_NB','CAR'),  add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, title = 'Data Generated with CAR model', include_legend = F, squeeze_plots = T, remove_x_axis = F, remove_y_axis = F, y_lab = 'MCAR')
  
  cowplot::plot_grid(p1$plot, p2$plot, p3$plot, p1$legend, ncol = 1, rel_heights = c(3,3,3,1))
  ggsave('figures/DGP_NB_district_comparison_plots_WFfreqnsample2000_07092024.pdf', height = 7.5, width = 5)
}
...
DGP_ylims = list(ylim(0,1), ylim(0,1), ylim(0, 1), ylim(0,1))
{
  pWF1 <- plot_all_methods(res = res_facility[["files_WF_MCAR_beta6_n025"]]$results, fix_axis = DGP_ylims, add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, title = 'Data Generated with WF model', include_legend = F, squeeze_plots = T, remove_x_axis = T, remove_y_axis = F, y_lab = 'MCAR')
  
  pWF2 <- plot_all_methods(res = res_facility[["files_WF_MAR_beta6_n025"]]$results, fix_axis = DGP_ylims, add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, include_legend = F, squeeze_plots = T, remove_x_axis = T, remove_y_axis = F, y_lab = 'MAR')
  
  pWF3 <- plot_all_methods(res = res_facility[["files_WF_MNAR_beta6_n025"]]$results, fix_axis = DGP_ylims, add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, include_legend = F, squeeze_plots = T, remove_x_axis = F, remove_y_axis = F, y_lab = 'MNAR')
  
  pfreq1 <- plot_all_methods(res = res_facility[["files_freqglm0202_MCAR_beta55_n025"]]$results, fix_axis = DGP_ylims, add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, title = 'Data Generated with freqGLM model', include_legend = F, squeeze_plots = T, remove_x_axis = T)
  
  pfreq2 <- plot_all_methods(res = res_facility[["files_freqglm0202_MAR_beta55_n025"]]$results, fix_axis = DGP_ylims, add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, include_legend = F, squeeze_plots = T, remove_x_axis = T)
  
  pfreq3 <- plot_all_methods(res = res_facility[["files_freqglm0202_MNAR_beta55_n025"]]$results, fix_axis = DGP_ylims, add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, include_legend = F, squeeze_plots = T)
  
  pCAR1 <- plot_all_methods(res = res_facility[["files_CAR33025_MCAR_beta6_n025"]]$results, fix_axis = DGP_ylims, add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, title = 'Data Generated with CAR model', include_legend = F, squeeze_plots = T, remove_x_axis = T)
  
  pCAR2 <- plot_all_methods(res = res_facility[["files_CAR33025_MAR_beta6_n025"]]$results, fix_axis = DGP_ylims, add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, include_legend = F, squeeze_plots = T, remove_x_axis = T)
  
  pCAR3 <- plot_all_methods(res = res_facility[["files_CAR33025_MNAR_beta6_n025"]]$results, fix_axis = DGP_ylims, add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, include_legend = F, squeeze_plots = T)
  
  cowplot::plot_grid(pWF1$plot, pWF2$plot, pWF3$plot, 
                     pfreq1$plot, pfreq2$plot, pfreq3$plot,
                     pCAR1$plot, pCAR2$plot, pCAR3$plot,
                     byrow = F) %>%
    cowplot::plot_grid(pWF1$legend, 
                       ncol = 1, 
                       rel_heights = c(9,1))
  ggsave('figures/TEST_06252024.pdf', height = 7.5, width = 12)
}
...
DGP_ylims = list(ylim(0,1), ylim(0,1), ylim(0, 1), ylim(0,1))
{
  p1 <- plot_all_methods(res = res_facility[["files_freqglm0202_MCAR_beta55_n025"]]$results, fix_axis = DGP_ylims, add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, title = 'Data Generated with freqGLM model, assuming MCAR', include_legend = F)
  
  p2 <- plot_all_methods(res = res_facility[["files_freqglm0202_MAR_beta55_n025"]]$results, fix_axis = DGP_ylims, add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, title = 'Data Generated with freqGLM model, assuming MAR', include_legend = F)
  
  p3 <- plot_all_methods(res = res_facility[["files_freqglm0202_MNAR_beta55_n025"]]$results, fix_axis = DGP_ylims, add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, title = 'Data Generated with freqGLM model, assuming MNAR', include_legend = F)
  
  cowplot::plot_grid(p1$plot, p2$plot, p3$plot, p1$legend, ncol = 1, rel_heights = c(3,3,3,1))
  ggsave('figures/DGP_freqGLM_facility_comparison_plots_06252024.pdf', height = 7.5, width = 7)
}

# CAR
DGP_ylims = list(ylim(0,1), ylim(0,1), ylim(0, 1), ylim(0,1))
{
  p1 <- plot_all_methods(res = res_facility[["files_CAR33025_MCAR_beta6_n025"]]$results, fix_axis = DGP_ylims, add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, title = 'Data Generated with CAR model, assuming MCAR', include_legend = F)
  
  p2 <- plot_all_methods(res = res_facility[["files_CAR33025_MAR_beta6_n025"]]$results, fix_axis = DGP_ylims, add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, title = 'Data Generated with CAR model, assuming MAR', include_legend = F)
  
  p3 <- plot_all_methods(res = res_facility[["files_CAR33025_MNAR_beta6_n025"]]$results, fix_axis = DGP_ylims, add_lines = list(F, F, F, F),  metrics = c('specificity', 'outbreak_detection3'), metric_rename = c('specificity', 'sensitivity-3'), results_by_point = F, rows = 1, title = 'Data Generated with CAR model, assuming MNAR', include_legend = F)
  
  cowplot::plot_grid(p1$plot, p2$plot, p3$plot, p1$legend, ncol = 1, rel_heights = c(3,3,3,1))
  ggsave('figures/DGP_CAR_facility_comparison_plots_06252024.pdf', height = 7.5, width = 7)
}

#
#### Processing Results from WF MCAR, MAR and MNAR (w/o QP and with higher nsample and burnin) ####
files <- grep('2024_06_24',dir(res_dir, full.names = T), value = T)
files2 <- grep('2024_06_20',dir(res_dir, full.names = T), value = T)

files_WF_MCAR_beta6_n025 <- grep('wf', files2, value = T)

files_WF_MAR_beta6_n025 <- grep('mar', files, value = T)

files_WF_MNAR_beta6_n025 <- grep('mnar', files, value = T)
...
file_name_str <- c('files_WF_MCAR_beta6_n025',
                   'files_WF_MAR_beta6_n025',
                   'files_WF_MNAR_beta6_n025')
...
# save(res_facility, res_district, file = 'C:/Users/Admin-Dell/Dropbox/Academic/HSPH/Research/Syndromic Surveillance/results/DGP_WF_and_freqGLM_and_CAR_results_06262024.RData')

#
#### Processing Results from freqGLM MCAR, MAR, and MNAR as well as CAR MCAR, MAR, and MNAR ####
