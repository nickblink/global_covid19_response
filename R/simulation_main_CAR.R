...
one_run <- function(lst, i, models = c('freq', 'WF', 'CAR')){
  ...
  if('CAR' %in% models){
    ...
    return_list <- tryCatch({
      res <- CARBayes_wrapper(return_list[['df_miss']], burnin = 10000, n.sample = 20000, prediction_sample = T, model = 'facility_fixed', predict_start_date = '2016-01-01', thin = 10)
    ...
    return_list <- tryCatch({
      res <- CARBayes_wrapper(return_list[['df_miss']], burnin = 10000, n.sample = 20000, prediction_sample = T, model = 'facility_fixed', predict_start_date = '2016-01-01', thin = 20)
      return_list[['df_miss']] <- res$df
      return_list[['district_df']] <- merge(return_list[['district_df']], res$district_df, by = c('district','date'))
    ...
    return_list <- tryCatch({
      res <- CARBayes_wrapper(return_list[['df_miss']], burnin = 20000, n.sample = 50000, prediction_sample = T, model = 'facility_fixed', predict_start_date = '2016-01-01', thin = 10)
      return_list[['df_miss']] <- res$df
      return_list[['district_df']] <- merge(return_list[['district_df']], res$district_df, by = c('district','date'))
    ...
    return_list <- tryCatch({
      res <- CARBayes_wrapper(return_list[['df_miss']], burnin = 20000, n.sample = 50000, prediction_sample = T, model = 'facility_fixed', predict_start_date = '2016-01-01', thin = 50)
      return_list[['df_miss']] <- res$df
      return_list[['district_df']] <- merge(return_list[['district_df']], res$district_df, by = c('district','date'))
