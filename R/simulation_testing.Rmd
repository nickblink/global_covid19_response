...
# simulation function!
df_miss = MNAR_sim(df, p = 0.5, by_facility = T)
...
p6 <- plot_county_fits(county_miss, imp_vec, color_vec, title = 'p = 0.1')
...
rho = 0.5
alpha = 0.5
# V = matrix(c(1, p, p^2, p, 1, p, p^2, p ,1), nrow = 3)
# 
# mat = MASS::mvrnorm(10000, rep(0,3), V)
# cor(mat)

df = df %>% arrange(facility, date)

# so let's just try temporal
autocor_mat = autocorr.mat(length(unique(df$date)), rho = rho)
spatial_mat = autocor_mat*alpha

test = MASS::mvrnorm(10000, rep(0,48), autocor_mat)

for(d in unique(df$district)){
  tmp = df %>% filter(district == d)
  n = length(unique(tmp$facility))
  V = blockmatrix::blockmatrix(names = c('AR','SP','SP','AR'), list(autocor_mat, spatial_mat, spatial_mat, autocor_mat))
}
