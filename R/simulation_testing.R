...
#### Profiling freqEPI CCA ####
lst <- simulate_data(district_sizes = c(4), R = 2, end_date = '2020-12-01')

df <- lst$df_list[[1]]

# simulation function!
df_miss <- MAR_spatiotemporal_sim(df, p = 0.2, rho = 0.8, tau = 3)
...
Rprof(freqGLMepi_list = freqGLMepi_CCA(df_miss, R_PI = 100, verbose = F))

#### 10/14/2022: WF Baseline, WF CCA, and CAR CCA with MAR ####
...
# save(imputed_list, file = 'results/simulation_noST_MARp2_R200_10152022.RData')

# Testing some missingness
df_miss %>% dplyr::filter(district == 'A') -> tt
tt <- tt %>% select(date, facility, y)
tt <- tidyr::spread(tt, key = facility, value = y)
ss <- is.na(tt[,-1])
cor(ss)

imp_vec = c("y_pred_baseline_WF", "y_pred_CCA_WF", "y_pred_CCA_CAR")
rename_vec = c('WF full data', 'WF CCA', 'CAR CCA')
