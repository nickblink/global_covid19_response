    ...
    if (counts_only==FALSE){
      ...
      tryCatch({
        ...
        if(sum(is.na(data.new$denom))>0){
          ...
          # create bootstrapped samples of imputed denominator values.
...
fit.site.specific.denom.pi.bayes.imp <-function(data,site_name,extrapolation_date,
                                      indicator_var,site_var,date_var,denom_var=NULL,
                                      level="month",counts_only=FALSE,R=250){
  if(counts_only==FALSE & is.null(denom_var)){
    warning("You must supply a denominator variable")
...
fit.cluster.pi <- function(data,
  ...
  print(sprintf('including %s out of %s sites', length(unique(data_filtered$facility)), length(unique(data$facility))))
